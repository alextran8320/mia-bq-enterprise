# MIABOS Portal CRM - Module And Sitemap Recommendation
...
**Scope**: Giay BQ / MIABOS Portal CRM
**Document Role**: Quyết định khuyến nghị về bộ module cần có và sitemap Portal CRM sau vòng phản biện chéo kiến trúc, sản phẩm, và chiến lược

---

## 1. Executive Decision

Portal CRM của MIABOS cho Giày BQ nên được thiết kế trên 3 lớp:
...
- Nhưng để đủ chặt cho build thực tế và đủ rõ cho Portal CRM, cần bổ sung thêm:
  - `I06 Projection / Read Model / Landing`
  - `M13 Channel / Branch / Dealer Operations`
  - `M14 Business Analytics / ROI`
- `Commercial Packaging / Tenant Onboarding` là capability platform quan trọng, nhưng không phải module người dùng chính của Portal CRM cho phase Giày BQ hiện tại.
...
| `I05` | Canonical Mapping And Source Of Truth | Keep | Map đa mã, ưu tiên nguồn dữ liệu, xử lý conflict |
| `I06` | Projection / Read Model / Landing | Add | Chuẩn hóa, materialize, replay, freshness, staging và landing cho dữ liệu tiêu thụ xuống business modules |
...
| `M04` | Promotion | Keep | CTKM, discount, voucher, coupon, promo scope |
...
| `M11` | Escalation And Workflow | Keep with workflow expansion | Escalation, ticket, assignment, workflow handoff |
...
|------------|--------|---------|
| `Commercial Packaging / Tenant Onboarding` | Add Later | Cần cho scale product và GTM, nhưng không phải top-level Portal CRM module cho BQ phase hiện tại |

---

## 3. Module Packaging For Portal CRM

Không nên expose toàn bộ module dưới dạng menu top-level. Portal CRM nên đóng gói chúng thành các product surfaces sau:
...
## 4. Portal CRM Sitemap Recommendation

## 4.1 Dashboard

| Page | Purpose | Primary Persona | Mapped Modules | Phase |
|------|---------|-----------------|----------------|-------|
| `Executive Dashboard` | Tổng quan vận hành CRM, AI, commerce, KPI chính | Owner / PM / HQ | `M14`, `M12` | P2 |
...
| `Approval Center` | Phê duyệt thay đổi giá, CTKM, knowledge publish, rule changes | HQ / Manager / Governance | `M11`, `M03`, `M04`, `M08`, `M07` | P2 |
...
| `Source Of Truth And Mapping` | Map đa mã và xử lý conflict dữ liệu | Tech Ops / PM / Ops Governance | `I05`, `I06`, `M13` | P1 |

## 4.8 Insights And Performance

| Page | Purpose | Primary Persona | Mapped Modules | Phase |
|------|---------|-----------------|----------------|-------|
| `Business Performance Dashboard` | Hiệu quả vận hành, sell-through, promo performance, branch compare | HQ / Regional Manager | `M14`, `M13`, `M03`, `M04`, `M05` | P2 |
...
- `I05 Canonical Mapping And Source Of Truth`
- `I06 Projection / Read Model / Landing`
...
- Thiết kế Portal CRM theo `product surfaces`, không expose raw module tree làm menu

### 6.2 Do Not Approve Yet

- Không xem module tree hiện tại là fully implementation-ready nếu chưa có `I06`
- Không rollout sales-facing rộng nếu chưa chốt `M07`, `I05`, `M03`, `M04`, `M12`
