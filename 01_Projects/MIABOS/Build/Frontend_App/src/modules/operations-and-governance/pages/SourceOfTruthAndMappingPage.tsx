...
type StatusFilter = "all" | "attention" | "active" | "resolved";

const STATUS_FILTERS: { value: StatusFilter; label: string }[] = [
  { value: "all", label: "Tất cả" },
  { value: "attention", label: "Đang xử lý" },
  { value: "active", label: "Cần chú ý" },
  { value: "resolved", label: "Đã xử lý" },
];

const columns: Column<MappingConflictRecord>[] = [
  {
    key: "id",
    header: "Mã",
    render: (r) => (
  ...
  },
  {
    ...
    render: (r) => (
      <Badge label={r.entityType} color="#7C3AED" bg="#F3E8FF" />
    ),
  },
  {
    key: "conflictReason",
    header: "Lý do xung đột",
    render: (r) => (
      <span
        style={{
          fontSize: "13px",
  ...
  },
  {
    ...
    render: (r) => <span style={{ fontSize: "13px" }}>{r.owner}</span>,
  },
  {
    key: "statusLabel",
    header: "Trạng thái",
...
export function SourceOfTruthAndMappingPage() {
  ...
  const records = MAPPING_CONFLICT_RECORDS.filter((r) => {
    if (statusFilter !== "all" && r.statusGroup !== statusFilter) return false;
    if (debouncedSearch.trim()) {
      const kw = normalize(debouncedSearch);
      const haystack = [r.id, r.canonicalKey, r.entityType, r.owner, r.conflictReason].map(normalize);
  ...
  return (
    ...
    >
      ...
      >
        <Input
          icon={<Search size={16} />}
          placeholder="Tìm mã conflict, canonical key, loại thực thể..."
          value={search}
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        <DataTable
          ...
          data={records}
          rowKey={(r) => r.id}
          onRowClick={(r) => navigate(`/operations/source-mapping/${r.id}`)}
        />
      </Card>
    </div>
  );
