...
| Câu hỏi | Quyết định |
|---------|-----------|
| Trigger condition phase 1 | **Hai điều kiện song song:** (1) AI confidence thấp → tự hiện CTA; (2) Nhân viên bấm chủ động bất kỳ lúc nào |
| Destination routing priority | **Domain/nghiệp vụ trước** (giá, đổi trả, kỹ thuật…) → nếu không có người phụ trách domain → Unrouted |
| Auto-create knowledge gap? | Không áp dụng trong phase 1; để manual |

**Impact lên UXUI / FE:**
- CTA "Cần người hỗ trợ" hiện theo 2 trigger: auto (confidence thấp) + manual (user bấm)
- Routing logic: match theo `domain tag` của escalation trước, không phải branch
- Unrouted state vẫn giữ nguyên; xảy ra khi không có owner domain nào available
