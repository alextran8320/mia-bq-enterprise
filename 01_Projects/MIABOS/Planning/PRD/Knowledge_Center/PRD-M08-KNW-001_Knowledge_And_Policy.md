...
UX patterns bắt buộc cho detail panel và AI answer payload (xem [RES-M08-KNW_UX_Patterns_And_IA.md](../../../Research/Knowledge_Center/RES-M08-KNW_UX_Patterns_And_IA.md)):
- **Source Citation Block**: tên tài liệu + version/ngày cập nhật + owner + freshness.
- **Scope Declaration**: answer phải nói rõ đang dựa trên Knowledge Base nội bộ / document scope nào, không giả định đang search internet hoặc catalog data.
- **Uncertainty Signal**: nếu thiếu source hợp lệ hoặc confidence thấp, payload trả state `uncertain` kèm escalation/contact action.
- **Stale Content Warning**: badge/warning khi document hoặc source vượt freshness threshold.
- **Quick Reply Chips**: knowledge item được phép khai báo related questions/topics để M09 gợi ý bước tiếp theo.
- **Role-Aware Response Header**: cùng một document có thể có guidance khác nhau theo role/persona/channel.
- **Feedback Loop**: document detail và answer reference có feedback action; M08 chỉ cung cấp entry point, analytics sâu thuộc M12 nếu bật.
...
- Trả `answer-ready metadata` cho M09/M10: summary tối đa 3-4 dòng, source citation, scope statement, uncertainty/stale state, quick replies, role-aware guidance, escalation owner, và feedback action
...
- Runtime answer không được trả confident answer nếu thiếu source hợp lệ; phải chuyển sang uncertainty signal + escalation path
- Answer summary phải ngắn, có cấu trúc, và dùng detail/library link cho phần dài thay vì dump toàn bộ tài liệu

### 9.4 Technical Constraints for Downstream Teams

- Cần contract rõ giữa `M08` và `M09` cho document/version/source reference
- Contract `M08 -> M09` phải hỗ trợ 3 interaction modes: `Instant Answer`, `Guided Process` cho SOP, và `Exploration` cho overview/drill-down
...
| 2026-04-17 | Align Knowledge Core với `RES-M08-KNW_UX_Patterns_And_IA`: M08 phải cung cấp answer-ready metadata cho M09, không chỉ lưu document body | Codex CLI / A01 PM Agent | Đảm bảo AI Chat có citation, uncertainty, stale warning, quick replies, role-aware guidance, escalation, và feedback loop |
